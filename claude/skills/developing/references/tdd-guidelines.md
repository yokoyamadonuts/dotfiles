...
テストを書いた後、**そのテストが本当に意図した動作を検証しているか**を確認する。テストがパスしていても、実際には何もテストしていない可能性がある。

### 1. 失敗すべき時に失敗するか確認（Mutation Testing の考え方）

テストの信頼性を検証する最も効果的な方法：
...
// → テストが FAIL になれば、元のアサーションは有効
// → テストが PASS なら、result.error が実際には設定されていない
```

### 2. 各テストケースが独立したデータを使用しているか確認

Table-driven tests や parameterized tests では、各ケースが**固有のデータ**を使用していることを確認：

```
確認方法:
1. 各テストケースにログ/デバッグ出力を追加
2. 出力が期待通りのバリエーションを持つか確認
3. 全ケースで同じ値が出力される場合 → データ共有の問題
```

### 3. テスト実行時の観察ポイント

- **実行されるテスト数**: 期待する数のテストが実行されているか
- **実行時間**: 異常に速い場合、実際にはスキップされている可能性
- **各テストの独立性**: 実行順序を変えても結果が同じか
...
- [ ] 正常系と異常系の両方をテストしているか
- [ ] 異常系テストでエラーが実際に返されるか確認したか
- [ ] 各テストケースで異なる入力値を使用しているか
- [ ] アサーションが具体的で意味のある検証をしているか
- [ ] テストを壊した時に適切に失敗するか確認したか
