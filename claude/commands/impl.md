...
Kent BeckのTDD方法論と高凝集度・低結合度・コロケーションの原則に従って開発を行います。
RED→GREEN→REFACTORサイクルをテストファーストアプローチで厳格に遵守します。

> **関連スキル**
> - `developing` - TDDワークフロー全体と設計原則の詳細ガイドライン（親スキル）
> - `writing-tests` - テスト作成のベストプラクティス（サブスキル）
>
> 本コマンドは`developing`スキルのワークフローを実行する。アーキテクチャ設計の詳細はスキルを参照。
